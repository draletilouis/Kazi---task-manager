...
export async function updatedProject(req, res) {
    try {
        ...
        const result = await projectService.updateProject(projectId, workspaceId, userId, req.body);
        res.status(200).json(result);
    } catch (error) {
        res.status(400).json({ error: error.message });
    }
...
export async function deleteProject(req, res) {
    try {
        ...
        const workspaceId = req.params.workspaceId;
        const projectId = req.params.projectId;
        const result = await projectService.deleteProject(projectId, workspaceId, userId);
        res.status(200).json(result);
    } catch (error) {
        res.status(400).json({ error: error.message });
    }
